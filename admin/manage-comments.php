...
$comment_query = "SELECT * FROM comments WHERE approved=false ORDER BY date_time DESC";
$pending_comments = mysqli_query($connection, $comment_query);
?>
      ...
      <div class="container dashboard__container">
          ...
          <aside>
              <ul>
                  ...
                  <li>
                      <a href="<?= ROOT_URL ?>admin/manage-comments.php" class="active">
                          <i class="uil uil-list-ul"></i>
          ...
          <h2>Pending Comments</h2>
              <?php if (mysqli_num_rows($pending_comments) > 0): ?>
                  <table>
                      ...
                      <tbody>
                          <?php while ($comment = mysqli_fetch_assoc($pending_comments)) : ?>
                              <tr>
                                  <td><?= htmlspecialchars($comment['author']) ?></td>
                                  <td><?= htmlspecialchars($comment['email']) ?></td>
                                  <td><?= htmlspecialchars($comment['body']) ?></td>
                                  <td><a href="<?= ROOT_URL ?>admin/approve-comment.php?id=<?= $comment['id'] ?>" class="btn sm">Approve</a></td>
                                  <td><a href="<?= ROOT_URL ?>admin/delete-comment.php?id=<?= $comment['id'] ?>" class="btn sm danger">Delete</a></td>
                              </tr>
                          <?php endwhile ?>
                      </tbody>
                  </table>
              <?php else : ?>
                  <div class="alert alert__message error">No pending comments</div>
              <?php endif ?>
        </div>      
    </main>
  </section>
